...
- `ExternalRow` — 홈 피드의 외부 글 한 줄 (날짜 + 출처 + 제목)
- `PostLayout` — 포스트 헤더 + 사이드바(날짜/카테고리/태그) + 본문 2컬럼 그리드
- `PostRow` — 목록/카테고리 페이지 공용 글 목록 행
- 페이지: `/`, `/2`…`/N`(피드), `/posts/[slug]`, `/category/[category]`, `/tag/[tag]`, `/tags`, `/search`,
...
홈은 **글 목록만** 보여준다. 원래 큰 헤드라인과 카테고리 나열 문구가 있었으나 목록 진입을
가로막아 걷어냈다. 대신 `h1`은 `.sr-only`로 남긴다 — `h1` 없는 페이지는 스크린리더 탐색과
SEO에서 손해라, 화면에서만 감춘다.

**홈 피드는 이 블로그 글 + 외부 글을 날짜순으로 합친다** (`src/pages/[...page].astro`). 두 종류를
같은 줄로 그리면 뭐가 원본 글인지 흐려지므로 형태를 달리했다.

- 이 블로그 글 → `PostRow`: 큰 제목 + 설명 + 태그
- 외부 글 → `ExternalRow`: 날짜 + 출처 배지 + 제목 한 줄 + `↗` (muted 톤)

정렬은 `sortKey`(`YYYY-MM-DD`)로 두 날짜 포맷(`Date` / `2026.05.07`)을 맞춰 비교한다.

**페이지네이션**: 30개씩, Astro `paginate()`로 `/`, `/2` … `/11`을 생성한다. 308개를 한 페이지에
쏟으면 무겁고, 외부 항목이 한 줄짜리라 30개는 스크롤 한두 번에 훑을 수 있는 양이다.
**2페이지부터는 `noindex`** — 원본 콘텐츠 없이 링크만 있는 목록이라 검색 색인 가치가 없다.
사이트맵에서도 같은 기준으로 제외해 Search Console 경고를 피한다.

주의: `getStaticPaths`는 컴포넌트 스코프 밖으로 hoist되므로 프론트매터에 선언한 상수를 참조할 수
없다 (`PAGE_SIZE`를 함수 안에 둔 이유).
...
- 홈 피드: 11페이지 생성 확인, 1페이지 30행 / 마지막 8행(308 = 30×10+8), 이전·다음 링크가 양
  끝에서 알맞게 사라지는지 확인, 2페이지 이상 `noindex` + 사이트맵 제외 확인. 모바일에서 외부
  행의 `↗`가 홀로 세 번째 줄에 떨어지던 것을 발견해 ≤560px에서 숨김 처리
- 검색 버튼: `$ /`가 검색으로 안 읽힌다는 지적에 돋보기 SVG + "검색" 라벨로 교체, 실제 렌더
  크기(13×13)와 링크 대상 확인
